...
void PlotGraph(vector <double> *, vector <double> *, vector <double> *, vector <double> *, const int, int *, TString, const char *, TString, int);
...
void PlotGraph(vector <double> *data_x, vector <double> *data_y, vector <double> *data_xerr, vector <double> *data_yerr, const int n, int * availableCurrents, TString legtitle, const char * outputtxt, TString outputpdf, int chipnr) {
	...
	for(int i = 0; i < n; i++) {
		...
		if(n_size[i] != 0) {
		    ...
		    p1fit->SetParameter(1,50);
		    p1fit->SetParLimits(1,40,50);
		    graph[i]->Fit("p1fit","R");
		    //graph[i]->Fit("pol1","R");
